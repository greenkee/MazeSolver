...
def replanASTAR(start, goalCoord, walls, robotWidthAndBuffer): #walls is made up of LineStrings
  path = []
...
def handleIntersection(currWall, currNode, walls, goal, openStack, addedWalls, robotWidthAndBuffer):
        ...
        if(((pAngle > Utils.tau/8.0) and (pAngle <= 3.0*Utils.tau/8.0)) or ((pAngle > 5.0*Utils.tau/8.0) and (pAngle <= 7.0*Utils.tau/8.0)) ): #upper/lower quadrant
            if(x==0): #first point
                if(dX < 0): #point 2 xVal greater than point 1's
                    addRoundNode(currWall, pointList[x], 3, currNode, walls, goal, openStack, addedWalls, robotWidthAndBuffer) #0 is below, 1 is right, 2 is above, 3 is left
                else: #point 1 xVal greater than point 2's
                    addRoundNode(currWall, pointList[x], 1, currNode, walls, goal, openStack, addedWalls, robotWidthAndBuffer)
            else: #second point
                if(dX < 0): #point 2 xVal greater than point 1's
                    addRoundNode(currWall, pointList[x], 1, currNode, walls, goal, openStack, addedWalls, robotWidthAndBuffer) #0 is below, 1 is right, 2 is above, 3 is left
                else: #point 1 xVal greater than point 2's
                    addRoundNode(currWall, pointList[x], 3, currNode, walls, goal, openStack, addedWalls, robotWidthAndBuffer)
        else: #means it's to the side of the robot
            if(x==0): #first point
                if(dY < 0): #point 2 yVal greater than point 1's
                    addRoundNode(currWall, pointList[x], 0, currNode, walls, goal, openStack, addedWalls, robotWidthAndBuffer) #0 is below, 1 is right, 2 is above, 3 is left
                else: #point 1 higher than point 2
                    addRoundNode(currWall, pointList[x], 2, currNode, walls, goal, openStack, addedWalls, robotWidthAndBuffer)
            else: #second point
                if(dY < 0): #point 2 yVal greater than point 1's
                    addRoundNode(currWall, pointList[x], 2, currNode, walls, goal, openStack, addedWalls, robotWidthAndBuffer) #0 is below, 1 is right, 2 is above, 3 is left
                else: #point 1 higher than point 2
                    addRoundNode(currWall, pointList[x], 0, currNode, walls, goal, openStack, addedWalls, robotWidthAndBuffer)
...
def checkAndAddIntersection(currLine, currNode, walls, goal, openStack, addedWalls, robotWidthAndBuffer): #whether point should be added if no intersection or not
    ...
    if(len(intersections) > 0):
        ...
        if(addedWalls[currWall] == 0):
            ...
            handleIntersection(currWall, currNode, walls, goal, openStack, addedWalls, robotWidthAndBuffer)
        return True
    else:
        return False
...
def addRoundNode(currWall, nextCoord, direction, currNode, walls, goal, queue, addedWalls, robotWidthAndBuffer): #endpoint, currNode, walls, goal, openStack
  ...
  if( checkAndAddIntersection(currLine, currNode, walls, goal, queue, addedWalls, robotWidthAndBuffer)):#find closest line, add endpoints instead
      print "CURR -> EP INTERSECT"
      intersection = True

  #check if endpoint to targetpoint intersects walls
  currLine = sg.LineString([endpoint, targetPoint])
  if( checkAndAddIntersection(currLine, currNode, walls, goal, queue, addedWalls, robotWidthAndBuffer)):#find closest line
